...
class PortfolioEngine:
    ...
    def get_rebalance_preview(self, as_of_date: str = None):
        ...
        # Primary cash display: aggregate all multi-currency cash into the base currency pool
        primary_cash_local = round(total_cash_base, 2)
    ...
    def rebalance(self, as_of_date: str = None):
        ...
        for trade in preview:
            # ── GUARD: Skip cash pseudo-row (shares=None, price=None) ──
            if trade.get('is_cash'):
                continue
            if trade.get('shares') is None or trade.get('price') is None:
                logger.warning(f"[Rebalance] Skipping trade with null shares/price: {trade.get('isin')}")
                continue
            # Insert only the BUY / SELL trade record.
            # NOTE: get_current_portfolio() already handles cash deduction/addition
            # automatically when it processes BUY/SELL events (lines ~84/94).
            # Do NOT write extra CASH_IN/CASH_OUT here — that would cause double-counting.
        ...
        self.conn.commit()
        # MUST load transactions first so that _auto_fx_sweep has the latest state in self.df
        self.load_transactions()
        # Trigger an auto FX sweep to clear out non-base wallets
        self._auto_fx_sweep(date_str)
        # Reload again to reflect the sweep transactions
        self.load_transactions()

    def _auto_fx_sweep(self, date_str: str):
        """
        After rebalancing or creating a portfolio, foreign asset buys will draw from foreign CASH naturally, 
        resulting in negative foreign cash balances. 
        This method sweeps these negative balances by transferring equivalent funds from the base currency wallet.
        """
        # Calculate true cash balances dynamically from ALL transactions
        cash_balances = {}
        for _, r in self.df.iterrows():
            ...
            if t == 'CASH_IN':
                key = isin if isin.startswith('CASH_') else f'CASH_{self.base_currency}'
                cash_balances[key] = cash_balances.get(key, 0) + p
            elif t == 'CASH_OUT':
                key = isin if isin.startswith('CASH_') else f'CASH_{self.base_currency}'
                cash_balances[key] = cash_balances.get(key, 0) + p  # p is already negative for CASH_OUT
            elif t == 'BUY' and not isin.startswith('CASH_'):
                currency = RealTime.guess_currency_from_isin(isin)
                c_key = f'CASH_{currency}'
                cash_balances[c_key] = cash_balances.get(c_key, 0) - q * p
        ...
        base_cash_isin = f"CASH_{self.base_currency}"
        cursor = self.conn.cursor()
        
        for c_key, balance in cash_balances.items():
            # Sweep any non-base currency wallets that are not zero (positive or negative)
            if abs(balance) > 0.01 and c_key != base_cash_isin: 
                foreign_ccy = c_key.replace("CASH_", "")
                exchange_rate = RealTime.get_fx_rate(foreign_ccy, self.base_currency) 
                
                if exchange_rate and exchange_rate > 0:
                    base_ccy_cost = abs(balance) * exchange_rate
                    if balance < -0.01:
                        # 1. Inject foreign currency to zero it out
                        cursor.execute("INSERT INTO transactions (portfolio_id, date, isin, type, shares, price) VALUES (?, ?, ?, 'CASH_IN', 1, ?)",
                                 (self.portfolio_id, date_str, c_key, abs(balance)))
                        # 2. Deduct the cost from the base currency (use NEGATIVE price for CASH_OUT)
                        cursor.execute("INSERT INTO transactions (portfolio_id, date, isin, type, shares, price) VALUES (?, ?, ?, 'CASH_OUT', 1, ?)",
                                 (self.portfolio_id, date_str, base_cash_isin, -base_ccy_cost))
                        logger.info(f"Auto FX Sweep (Deficit): OFFSET {c_key} ({balance:.2f}) -> COST {base_cash_isin} ({-base_ccy_cost:.2f}) at rate {exchange_rate:.4f}")
                    elif balance > 0.01:
                        # 1. Eject excess foreign currency to zero it out (use NEGATIVE price for CASH_OUT)
                        cursor.execute("INSERT INTO transactions (portfolio_id, date, isin, type, shares, price) VALUES (?, ?, ?, 'CASH_OUT', 1, ?)",
                                 (self.portfolio_id, date_str, c_key, -abs(balance)))
                        # 2. Add the proceeds to the base currency
                        cursor.execute("INSERT INTO transactions (portfolio_id, date, isin, type, shares, price) VALUES (?, ?, ?, 'CASH_IN', 1, ?)",
                                 (self.portfolio_id, date_str, base_cash_isin, base_ccy_cost))
                        logger.info(f"Auto FX Sweep (Surplus): EJECT {c_key} ({balance:.2f}) -> CREDIT {base_cash_isin} ({base_ccy_cost:.2f}) at rate {exchange_rate:.4f}")
    ...
    def get_dividend_projections(self, tax_rate: float = 0.0, drip: bool = False):
        ...
        for isin in isins:
            ...
            asset_currency = market_data.get('currency', 'USD') if market_data else 'USD'
            fx_rate = RealTime.get_fx_rate(asset_currency, base_ccy)
            base_price = curr_price * fx_rate
            ...
            # Apply FX Rate to normalize to Base Currency for Total Income Projection
        ...
        for d in dates:
            ...
            for isin, shares in daily_shares.items():
                if shares > 0:
                    ...
                    elif avg_cost and not pd.isna(avg_cost) and price < avg_cost:
                    ...
                    addition = shares * (price if price and not pd.isna(price) else 0)
                    daily_nav += addition
